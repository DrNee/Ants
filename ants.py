...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        return self.container and not (self.ant or other.container)
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        location, curr_range = self.place, 0
        while location and location != hive:
            if location.bees and (curr_range >= self.min_range and curr_range <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            bees = list(self.place.bees) # iterating over copy since original sequence is modified inside loop
            for bee in bees:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.impostor:
            ...
            behind_queen = self.place.exit
            while behind_queen:
                if behind_queen.ant:
                    if behind_queen.ant not in self.doubled:
                        self.doubled += [behind_queen.ant]
                        behind_queen.ant.damage *= 2
                    if isinstance(behind_queen.ant, BodyguardAnt) and behind_queen.ant.ant and behind_queen.ant.ant not in self.doubled:
                        self.doubled += [behind_queen.ant.ant]
                        behind_queen.ant.ant.damage *= 2
                behind_queen = behind_queen.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if not self.impostor and self.armor <= 0:
            return bees_win()
        # END Problem 13
